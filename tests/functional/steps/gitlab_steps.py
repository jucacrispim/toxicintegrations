...
def click_gitlab_import(context):
    ...
    btn = browser.wait_element_become_present(
        lambda: browser.find_element(By.CLASS_NAME, 'fa-gitlab'))
    btn.click()
...
def is_in_gitlab_login_page(context):
    browser = context.browser
    el = browser.find_element(By.ID, 'user_login')
...
def fill_passwd(context):
    browser = context.browser
    el = browser.find_element(By.ID, 'user_password')
...
def accept_cookies(context):
    ...
    def fn():
        el = browser.find_element(By.ID, 'onetrust-accept-btn-handler')
        return el
